...
SELECT DEP_ID, COUNT(*) AS  NUMBER_OF_EMPLOYEES , AVG(SALARY) AS AVERAGE_DEPARTMENT_SALARY 
FROM EMPLOYEES 
GROUP BY DEP_ID;
...
-- BONUS Query 6: Similar to 4B but instead of department ID use department name. Retrieve a list of employees ordered by department 
--   name, and within each department ordered alphabetically in descending order by last name.
